...
BasePath* YenTopKShortestPathsAlg::get_shortest_path(BaseVertex* pSource,
		BaseVertex* pTarget) {
...
BasePath* YenTopKShortestPathsAlg::next() {
	...
	BaseVertex* cur_derivation_pt =
			m_mpDerivationVertexIndex.find(cur_path)->second;
	...
	for (int i = 0; i < count - 1; ++i) {
		...
		//
		BaseVertex* cur_succ_vertex = cur_result_path->GetVertex(
				sub_path_length + 1);
		m_pGraph->remove_edge(
				make_pair(cur_derivation_pt->getID(),
	...
	for (int i = 0; i < path_length - 1; ++i) {
		...
		m_pGraph->remove_edge(
				make_pair(cur_path->GetVertex(i)->getID(),
						cur_path->GetVertex(i + 1)->getID()));
	}
	...
	for (int i = path_length - 2; i >= 0 && !is_done; --i) {
		...
		BasePath* sub_path = reverse_tree.update_cost_forward(
				cur_recover_vertex);
		...
		m_pGraph->recover_removed_edge(
				make_pair(cur_recover_vertex->getID(), succ_vertex->getID()));

		//4.6 Update cost if necessary
		double cost_1 = m_pGraph->get_edge_weight(cur_recover_vertex,
				succ_vertex) + reverse_tree.get_start_distance_at(succ_vertex);
		...
		if (reverse_tree.get_start_distance_at(cur_recover_vertex) > cost_1) {
			...
			reverse_tree.set_predecessor_vertex(cur_recover_vertex,
					succ_vertex);
			reverse_tree.correct_cost_backward(cur_recover_vertex);
		}
	}
